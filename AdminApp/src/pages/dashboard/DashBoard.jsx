...
import { formatNumberToVietNamCurrency, paginate } from "../../utils/common";
...
export default function DashBoard() {
  ...
  const quantityUser = users.length;
        ...
        {orders.map((item, index) => (
          ...
          <div key={index} className={`${styles["content"]}  row`}>
            ...
            <div className={`${styles["total-amount"]}`}>
              {formatNumberToVietNamCurrency(item.totalAmount).replace(
                "VND",
                ""
              )}
            </div>
  ...
  return (
    <div className={`${styles["dash-board-container"]} `}>
      ...
      <div className={`${styles["list-transaction"]}`}>
        ...
        {orders?.length > 0 && (
          <div>
            <p className={`${styles["title-list-orders"]} row`}>History</p>
            {orderListEL}
          </div>
        )}
      </div>
